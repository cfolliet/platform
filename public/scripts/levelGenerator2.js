...
function getTiles(width, height, tileSize) {
    ...
    while(iteration > 0){
        ...
        const maxW = Math.random() * width / (isVertical ? 10 : 4) | 0;
        const maxH = Math.random() * height / (isVertical ? 4 : 10) | 0;
        const w = Math.max(tileSize * 2, roundToSize(maxW, tileSize));
        const h = Math.max(tileSize * 2, roundToSize(Math.random() * maxH, tileSize));
